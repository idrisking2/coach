...
var tabs=[['dash','Dashboard'],['today','Heute'],['planner','Session-Builder'],['year','52 Wochen'],['blocks','Blöcke'],['log','Log'],['finger','Finger/Kraft'],['tech','Techniklabor'],['moon','MoonBoard'],['tests','Tests'],['analysis','Analyse'],['data','Daten']];
function base(){return{start:today,week:1,sessions:[],moon:[],tests:[],goals:[['5er zuverlässiger','75-85% in einer Session','aktiv'],['Fingergesund bleiben','keine Ringbandreaktion','aktiv'],['Mehr über Füße ziehen','weniger Cut-loose','aktiv']],timer:180}}
...
function safe(id,fn){try{fn()}catch(e){el(id).innerHTML='<div class="error"><b>Modulfehler</b><br>'+esc(e.message)+'</div>'}}
function render(){safe('pg_dash',UI.dash);safe('pg_today',UI.today);safe('pg_planner',UI.planner);safe('pg_year',UI.year);safe('pg_blocks',UI.blocks);safe('pg_log',UI.log);safe('pg_finger',UI.finger);safe('pg_tech',UI.tech);safe('pg_moon',UI.moon);safe('pg_tests',UI.tests);safe('pg_analysis',UI.analysis);safe('pg_data',UI.dataPage)}
